...
static int encrypt_symbol(Elf32_Ehdr *hdr, Elf32_Sym *sym, void *prefix) //prefix : _encrypt_
{
	int key_length;
	char *sname;
	char *b;
	...
	if (strstr(sname, prefix) == sname) { //_encrypt_ 's func to encrypt
		printf(LOG_PREFIX "Encrypting %s...\n", sname);
		addr = sym->st_value; //func's size 
		addr = addr - elf_dot_text_vaddr(hdr) + //func's addr
...
int main(int argc, char **argv)
{
	...
	sym_foreach(ehdr, encrypt_symbol, "_encrypt_");
	encrypt_rodata(ehdr);
